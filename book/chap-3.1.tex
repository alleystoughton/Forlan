...
we have made use of two base cases, so that
...
$L(\mathsf{(0+1)^3})=L(\mathsf{0+1})^3=\{\mathsf{0,1}\}^3$.
...
$L(\alpha)=L$.  Because $\Reg$ is countably infinite, it follows
that there is an injection from $\RegLan$ to $\nats$.  And
we already gave an injection from $\nats$ to $\RegLan$, completing
the proof that $\RegLan$ and $\nats$ have the same size, i.e.,
that $\RegLan$ is countably infinite.
...
meaning (it generates $\mathsf{001}$ and fails to generate $\%$.)
...
invoked directly, or run via Forlan.
See the Forlan website for more information.
