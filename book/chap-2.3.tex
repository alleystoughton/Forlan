...
Linux, Mac OS X and Windows can be found on the Forlan website:
\begin{center}
...
under Linux and Mac OS X, and \emph{CTRL}-\texttt{z} under Windows.
...
double-clicking on the Forlan icon.  On all platforms, a much more flexible
and satisfying way of running Forlan is as a subprocess of an IDE (integrated
development environment).  See the Forlan website for
...
Instead of using \texttt{<}, \texttt{<=}, \texttt{>} and \texttt{>=}
for comparing integers, SML offers a function \texttt{Int.compare} of
type \texttt{int~*~int~->~order}, where the \texttt{order} type
contains three elements: \texttt{LESS}, \texttt{EQUAL} and
\texttt{GREATER}.
\input{chap-2.3-forlan2c}
...
in local environment, evaluate an expression in that environment, and
...
E.g., we can define the list reversal function like this:
...
when called with \texttt{""}, and all of Forlan's
