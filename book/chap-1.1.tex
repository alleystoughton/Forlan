...
A=\emptyset=A\cap\emptyset$, for all sets $A$).  $A-B$ is formed by
removing the elements of $B$ from $A$, if necessary.  For example,
$\{0,1,2\}-\{1,4\}=\{0,2\}$.
...
We must show that $A\cap(B\cup C)\sub(A\cap B)\cup(A\cap C)\sub
A\cap(B\cup C)$.
...
If we allowed $\bigcap\emptyset$, then it would contain
all elements $a$ of our universe that are in all of the nonexistent
elements of $\emptyset$, i.e., it would contain all elements of
our universe.  It turns out, however, that there is no such
set, which is why we may only take generalized intersections
of non-empty sets.
...
where the pair from $R$ left off), and supressing the intermediate
...
of a relation $R$ ($R^{-1}$) is the relation $\setof{(y,
x)}{(x, y)\in R}$, i.e., it is the relation obtained by reversing each
of the pairs in $R$.  For example, if $R=\{(0, 1), (1, 2), (1, 3)\}$,
then the inverse of $R$ is $\{(1, 0), (2, 1), (3, 1)\}$.
So for all sets $A$ and $B$, and relations $R$, if $R$ is
a relation from $A$ to $B$, then $R^{-1}$ is a relation from
$B$ to $A$.
...
Futhermore, if $R$ is a total ordering on $A$, then $R^{-1}$ is also a
total ordering on $A$.
...
and $\range\,f\sub Y$.  We write $X\fun Y$
...
If the domain of $f$ is a product $X_1\times\cdots\times X_n$,
for $n\geq 1$ and
\index{product}%
...
\index{set!same size}%
($X\cong Y$)
\index{ same size@$\cong$}%
\index{set! same size@$\cong$}%
...
Every set $X$ has a \emph{size} or \emph{cardinality} ($|X|$)
\index{ size of@$\sizedot$}%
\index{set! size of@$\sizedot$}%
and we have that, for all sets $X$ and $Y$, $|X|=|Y|$ iff
$X\cong Y$.  The sizes of finite sets are natural numbers.
...
\index{diagonalization!cardinality}%
...
Thus, there is a bijection $f$ from $\nats$ to 
$\powset\,\nats$.  Define $X\in\setof{n\in\nats}{n\not\in f\,n}$,
so that $X\in\powset\,\nats$.  By the definition of $f$, it
follows that $X=f\,n$, for some $n\in\nats$.  There are two
cases to consider.
\begin{itemize}
...
\index{ no bigger than@$\preceq$}%
\index{set! no bigger than@$\preceq$}%
iff there is an injection from $X$ to $Y$, i.e., an
injective function whose domain is $X$ and whose range
is a subset of $Y$.  For example, the injection $\id_\nats$ shows that
...
$\nats\subsetneq\ints\subsetneq\reals$.  \index{set|)}%
